...
└── AIコーディングエージェント/  ✅ Claude Code を"開発の道具"として使い倒す側（生成AIとは逆の立ち位置）
    ├── README.md                   ← 索引・全体像・機能の早見
    ├── 01_claude_code基礎.md          ← 起動モード(対話/print/plan)/基本操作/コンテキスト管理
    ├── 02_スラッシュコマンドとスキル.md ← 定型作業を再利用可能な手順に固める
    ├── 03_サブエージェント.md          ← 文脈分離での並列調査・多視点レビュー
    ├── 04_フック.md                    ← Hooks（整形/ガード/検証を決定的に強制）
    ├── 05_設定とメモリ.md              ← settings.json/権限/CLAUDE.md/メモリ
    ├── 06_mcp連携.md                   ← 外部ツール/データの接続（使う側の運用）
    ├── 07_ワークフロー実践.md          ← 探索→計画→実装→検証→コミットの型/TDD/PR
    ├── 08_api_sdk連携.md               ← Claude API/Agent SDK でCI・自作ツールに組み込む側
    ├── 09_ベストプラクティス.md        ← 文脈/検証/最小権限の3原則とアンチパターン総覧
    └── 10_この環境の実例_ecc.md        ← この PC の設定(ECCプラグイン)の解説＝概念の答え合わせ
```
...
> **🤖 AIコーディングエージェント**：`生成AI/` が「LLMを**アプリに組み込んで作る**側」なのに対し、`AIコーディングエージェント/` は「**Claude Code を開発作業で使い倒す**側」（立ち位置が逆）。起動モード・スラッシュコマンド/Skill・サブエージェント・フック・設定/メモリ(CLAUDE.md)・MCP連携・plan mode を軸にした実践ワークフロー、API/SDK 連携までを既存の書式で整理。入口は [AIコーディングエージェント/README.md](./AIコーディングエージェント/README.md)。
