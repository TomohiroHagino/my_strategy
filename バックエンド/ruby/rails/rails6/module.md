...
## 自動読み込みとの関係（Zeitwerk）
Rails 6 から **Zeitwerk** が既定（`config.autoloader = :classic` で従来方式も選べる）。**フォルダ名＝モジュール名、ファイル名＝定数名**を厳密に対応させる。
```
app/services/payment/processor.rb  →  Payment::Processor（必須）
```
- ズレると `NameError`（"expected file ... to define constant ..."）。
- 頭字語（API 等）は `config/initializers/inflections.rb` の `inflect.acronym "API"` を設定。
- 詳細は [zeitwerk.md](./zeitwerk.md)。
...
## ハマりどころ / アンチパターン
- **フォルダと module 名のズレ**（Zeitwerk）：`app/models/api_client.rb` に `module API` と書くと NameError。→ [zeitwerk.md](./zeitwerk.md)
...
## 関連
[concern.md](./concern.md) / [zeitwerk.md](./zeitwerk.md) / [routing.md](./routing.md)
