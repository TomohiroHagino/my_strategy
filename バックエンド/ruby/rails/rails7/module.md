...
## 自動読み込みとの関係（Zeitwerk）
Rails 7 は **Zeitwerk** が既定（classic は廃止方向）。**フォルダ名＝モジュール名、ファイル名＝定数名**を厳密に対応させる。
```
app/services/payment/processor.rb  →  Payment::Processor（必須）
```
- ズレると `NameError`（"expected file ... to define constant ..."）。
- 頭字語（API 等）は `config/initializers/inflections.rb` で `inflect.acronym "API"` を設定し、`app/controllers/api/...` ↔ `API::...` を一致させる。
- `require` は基本書かない（自動）。
...
## ハマりどころ / アンチパターン
- **フォルダと module 名のズレ**（Zeitwerk）：`app/models/api_client.rb` に `module API` と書くと NameError。
