...
Rails.application.routes.draw do
  ...
  resources :posts do                     # 7アクション（index/show/new/create/edit/update/destroy）
    ...
    resources :comments, only: %i[create destroy]  # ネスト（深さは2まで）
  end
...
- `constraints` でサブドメインや書式制限、`direct`/`resolve` でカスタムURL。
- 確認は **`rails routes`**（`rails routes -g post` で絞り込み）。
...
- HTTPメソッド違い（`get` で破壊的操作を作らない。削除は `delete`）。
